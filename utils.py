...
from models import XUMX, NoaudioXUMX, _STFT, _Spectrogram


def load_model(model_name, device="cpu"):
    print("Loading model from: {}".format(model_name), file=sys.stderr)
    conf = torch.load(model_name, map_location="cpu", weights_only=False)
    if "noaudio" in model_name:
        model = NoaudioXUMX.from_pretrained(model_name)
    else:
...
def get_statistics(args, dataset):
    ...
    spec = torch.nn.Sequential(
        _STFT(window_length=args.window_length, n_fft=args.in_chan, n_hop=args.nhop, center=center),
